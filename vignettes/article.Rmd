...
乱数の生成に使う関数を独自に指定することができます。たの関数は返すべき配列の長さを引数`size`に受け取るのでなければなりません。また、こいつをトリガーする`do.call()`がデフォルトで`parent.frame()`が返す環境を見る仕様なので、手もとの環境にある関数でないと呼べません。
