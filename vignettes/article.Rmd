---
title: "Random string generation with nanoidr"
author: "Kato Akiru"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Random string generation with nanoidr}
...
いろいろと調べたり手探りで試したりしたところ、以下のような雰囲気のやり方でできました。ポイントとしては`var __global__ = this;`でグローバル自身への参照をつくったあとでselfを宣言すると期待するように動くらしいこと、`self.crypto.getRandomValues`で`console.r.call('myFunctionName', args)`でR側の`myFunctionName(args)`をトリガーすることでしょうか。

なおこれは余談ですが、公式のvignetteだと`V8::v8(typed_arrays = TRUE)`とするとTypedArrayオブジェクトが使える臭いことが書かれていますが、手もとのバージョン2.1の{V8}環境 on Windowsだとそんなものは使えませんでした。原因はよくわかりませんが、ようするに配列の長さがわかればよいので、ここでは渡された配列の長さをとってコールバック先に渡しています。
...
`openssl::rand_bytes(size)`で`size`長のArrayBufferをつくることができます。いわゆるArray of bytesなオブジェクトはR側だとraw vectorとかいうやつで、画像をごにょごにょするときなどにまれによく使わるものです。TypedArrayオブジェクトが使えないV8環境にraw vectorをそのまま持ちこもうとしてもキャストしてくれないので、ここではR側で`as.integer()`しています。とくにひねりはないです。
...
こんな感じです。ちなみに`generate()`と下で出てくる`format()`は`init.locales`にロケールをリストとして渡すことで、その語における~~えっちな単語~~などが生成される文字列中に偶然に含まれることを抑制することができます。
...
乱数の生成に使う関数を独自に指定することができます。ただし、この関数は返すべき配列の長さを引数`size`に受け取るのでなければなりません。また、こいつをトリガーする`do.call()`がデフォルトで`parent.frame()`が返す環境を見る仕様なので、手もとの環境にある関数でないと呼べません。
